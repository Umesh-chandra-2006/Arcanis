...
app OAuth requires cookies to maintain session state. If a user's browser blocks cookies, authentication will not work.
...
const baseUrl = `https://myapp-${prefix}.app.space`;
const invitationUrl = `${baseUrl}/invite/${token}`;

// ❌ Same thing here, we should make sure that this information is preserved
const url = `https://${projectName}.app.space/callback`;
...
// Returns: "https://myapp.app.space" (no trailing slash)

// For specific paths
const callbackUrl = `${window.location.origin}/api/oauth/callback`;
// Returns: "https://myapp.app.space/api/oauth/callback"
```
...
export const startLogin = () => {
  ...
  // so a sibling *.app.space site cannot plant a matching value.
