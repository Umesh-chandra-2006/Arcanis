...
This template gives you a React 19 + Tailwind 4 + Express 4 + tRPC 11 stack with OAuth already wired. Procedures are your contracts, types flow end to end, and authentication "just works".
...
- **Auth baked in:** `/api/oauth/callback` handles OAuth, `protectedProcedure` injects `ctx.user`.
...
**Required workflow:**
1. Upload assets using the CLI upload command
2. Use the returned storage path directly in your code: `<img src="/app-storage/image_a1b2c3d4.png" />`
...
- OAuth completes at `/api/oauth/callback` and drops a session cookie.
...
- `BUILT_IN_FORGE_API_URL`: Built-in APIs (includes llm, storage, data_api, notification, etc...)
- `BUILT_IN_FORGE_API_KEY`: Bearer token used by Built-in APIs (server-side)
- `VITE_FRONTEND_FORGE_API_KEY`: Bearer token for frontend access to Built-in APIs
- `VITE_FRONTEND_FORGE_API_URL`: Built-in APIs URL for frontend
...
Use the preconfigured storage helpers in `server/storage.ts`. Credentials are injected from the platform (no manual setup required). Files are stored securely and served via the built-in `/app-storage/` path — no manual URL management needed.
...
// url = "/app-storage/{key}" — use directly in frontend code
...
- The returned `url` (e.g. `/app-storage/...`) is automatically served via signed redirect — no manual URL signing needed.
...
**CRITICAL: The proxy provides FULL access to ALL Google Maps features** - including advanced drawing, heatmaps, Street View, all layers, Places API, etc. Do ask users for Google Map API keys - authentication is automatic.
...
When you need external data, use the omni_search with search_type = 'api' to see there's any built-in api available in API Hub access. You only have to connect other api if there's no suitable built-in api available.
...
const url = `https://${projectName}.app.dev/callback`;
