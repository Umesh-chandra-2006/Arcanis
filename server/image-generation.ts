...
export async function generateSpellCardImage(
  ...
  // Concatenate locked template: [FIXED STYLE ANCHOR] + [DYNAMIC image_prompt] + [FIXED EXCLUSION SUFFIX]
  ...
  for (const providerUrl of HUGGINGFACE_PROVIDERS) {
    try {
      ...
      if (imageUrl) {
        // If it's a URL, fetch the image buffer and convert to base64 data URL
